...
test('generated filler and players whose id M27 ships never get a pack id', () => {
  ...
  const a = PortraitPackService.assign([{ PID: 0 }, { PID: 0 }], [filler, manning]);
  assert.equal(a.length, 0); // filler skipped; Manning's own id 1971 is shipped, so not ours to supply
});
