...
test('the pool is balanced like a class: edges are not nearly all LEDG, and off-ball backers not all MIKE', () => {
...
test('a roster add gets the slot the pool showed (a REDG stays REDG in a one-player class)', async () => {
  const cat = PlayerLookupService.catalog();
