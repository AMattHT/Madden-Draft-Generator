 ...
 *
 * Two sources of secondary roles:
 *   - a curated list (data/lookups/two-way-players.json): Baugh punted and played
 *     safety, Blanda kicked, Bednarik snapped, Deion caught passes;
...
export const TwoWayService = {
  ...
  rolesFor(first: string, last: string, draftYear: number, primaryPosId: number): TwoWayInfo | null {
    ...
    if (draftYear <= TWO_WAY_ERA_END) {
      const mirror = MIRROR[PositionMapper.groupFromId(primaryPosId)] ?? '';
