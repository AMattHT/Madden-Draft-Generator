...
export function FranchisePanel({
  ...
  // Free-agent trim
  const [faOvr, setFaOvr] = useState(65);
  const [faAgeOn, setFaAgeOn] = useState(false);
  const [faAge, setFaAge] = useState(32);
  const [faTargetOn, setFaTargetOn] = useState(false);
  const [faTargetN, setFaTargetN] = useState(400);
  const [faBusy, setFaBusy] = useState(false);
  const [faResult, setFaResult] = useState<FaTrimResult | null>(null);
  const [faError, setFaError] = useState<string | null>(null);

  // Draft-pick reset
  const [dpBusy, setDpBusy] = useState(false);
  const [dpResult, setDpResult] = useState<DraftPickResetResult | null>(null);
  ...
  // Reset per-save tool results when the selected save changes.
  useEffect(() => {
    setSchedule(null); setSchedError(null);
    setTrResult(null); setTrError(null);
    setFaResult(null); setFaError(null);
    setDpResult(null); setDpError(null);
  }, [selected]);
  ...
  async function runFaTrim(dryRun: boolean) {
    if (!selected) return;
    setFaBusy(true); setFaError(null);
    try {
      const res = await api.franchiseTrimFreeAgents(selected, {
        ovrThreshold: faOvr, ageThreshold: faAgeOn ? faAge : 0, targetN: faTargetOn ? faTargetN : 0, dryRun,
      });
      setFaResult(res);
      if (!dryRun) api.franchiseList().then((r) => setFiles(r.franchises)).catch(() => {});
    } catch (e) {
  ...
  async function runDraftPickReset(dryRun: boolean) {
    if (!selected) return;
    setDpBusy(true); setDpError(null);
    try {
      const res = await api.franchiseResetDraftPicks(selected, { dryRun });
      setDpResult(res);
      if (!dryRun) api.franchiseList().then((r) => setFiles(r.franchises)).catch(() => {});
    } catch (e) {
      setDpError((e as Error).message);
    } finally {
      setDpBusy(false);
    }
  }
  ...
  return (
    <div className="mx-auto flex h-full max-w-4xl flex-col gap-4 overflow-auto px-6 py-6">
      ...
      {/* Free-Agent Trim */}
      <div>
        <h1 className="text-xl font-bold tracking-tight">Trim Free Agents</h1>
        <p className="mt-1 text-xs text-muted">
          Thins a bloated free-agent pool by rating and age (the game keeps hundreds of low-overalls around).
          Only unsigned free agents are touched — signed players, practice squad, and the upcoming draft class
          are never affected. Preview first, then write a{' '}
          <code className="rounded bg-black/30 px-1">CAREER-…-FATRIM</code> file.
        </p>
      </div>
      ...
      <div className="rounded-lg border border-border bg-surface-1 p-4">
        <div className="flex flex-wrap items-end gap-4">
          <Field label="Cut below OVR" hint="e.g. 65 removes the low end">
            <input type="number" min={0} max={99} value={faOvr} onChange={(e) => setFaOvr(Number(e.target.value))} className={`${inputCls} max-w-[7rem]`} />
          </Field>
          <label className="flex items-center gap-2 text-sm text-neutral-200">
            <input type="checkbox" checked={faAgeOn} onChange={(e) => setFaAgeOn(e.target.checked)} />
            Also cut age ≥
            <input type="number" min={20} max={45} value={faAge} disabled={!faAgeOn} onChange={(e) => setFaAge(Number(e.target.value))} className={`${inputCls} max-w-[5rem] disabled:opacity-50`} />
          </label>
          <label className="flex items-center gap-2 text-sm text-neutral-200">
            <input type="checkbox" checked={faTargetOn} onChange={(e) => setFaTargetOn(e.target.checked)} />
            Trim pool down to
            <input type="number" min={0} max={750} value={faTargetN} disabled={!faTargetOn} onChange={(e) => setFaTargetN(Number(e.target.value))} className={`${inputCls} max-w-[6rem] disabled:opacity-50`} />
          </label>
        </div>

        <div className="mt-4 flex gap-3">
          <button
            onClick={() => runFaTrim(true)}
            disabled={faBusy || !selected}
          ...
          >
            {faBusy ? 'Working…' : 'Preview'}
          </button>
          <button
            onClick={() => runFaTrim(false)}
            disabled={faBusy || !selected || !faResult || faResult.trimmed === 0}
          ...
          >
            {faBusy ? 'Trimming…' : 'Trim → new save'}
          </button>
        </div>
      </div>

      {faError && (
        <div className="rounded-lg border border-danger/40 bg-danger/10 px-4 py-3 text-sm text-red-200">{faError}</div>
      )}
      ...
      {faResult && (
        <div className="rounded-lg border border-success/40 bg-success/10 p-4 text-sm">
          <div className="font-semibold text-green-100">
            {faResult.dryRun
              ? `Preview — ${faResult.trimmed} of ${faResult.freeAgentsBefore} free agents would be trimmed (pool ${faResult.freeAgentsBefore} → ${faResult.freeAgentsAfter})`
              : <>Wrote <code className="rounded bg-black/30 px-1">{faResult.output}</code> — trimmed {faResult.trimmed} (pool {faResult.freeAgentsBefore} → {faResult.freeAgentsAfter})</>}
          </div>
          <div className="mt-1 text-xs text-green-200/70">Pool cap is {faResult.maxFreeAgents} — nothing else needs updating.</div>
          {faResult.victims.length > 0 && (
          ...
          <div className="mt-2 text-xs text-green-200/70">
            {faResult.dryRun ? 'Looks right? Hit Trim to write the save.' : 'Load it in Madden (Franchise → Load).'}
          </div>
        </div>
      )}
      ...
      {/* Draft-Pick Reset */}
      <div>
        <h1 className="text-xl font-bold tracking-tight">Un-Trade Draft Picks</h1>
        <p className="mt-1 text-xs text-muted">
          Restores every future draft pick to the team that originally owned it — reverses all pick trades in
          one shot. Only the future-pick pool is touched; the current in-progress draft and completed
          selections are left alone. Preview first, then write a{' '}
          <code className="rounded bg-black/30 px-1">CAREER-…-DRAFTPICKS</code> file.
        </p>
      </div>
      ...
      <div className="rounded-lg border border-border bg-surface-1 p-4">
        <div className="flex gap-3">
          <button
            onClick={() => runDraftPickReset(true)}
            disabled={dpBusy || !selected}
            className="inline-flex items-center gap-2 rounded-md border border-border bg-surface-0 px-4 py-2 text-sm font-semibold text-neutral-200 transition-colors hover:border-primary disabled:opacity-50"
          >
            {dpBusy ? 'Working…' : 'Preview'}
          </button>
          <button
            onClick={() => runDraftPickReset(false)}
            disabled={dpBusy || !selected || !dpResult || dpResult.restored === 0}
            className="inline-flex items-center gap-2 rounded-md bg-primary px-4 py-2 text-sm font-semibold text-white transition-colors hover:bg-primary-dark disabled:opacity-50"
          >
            {dpBusy ? 'Restoring…' : 'Un-trade → new save'}
          </button>
        </div>
      </div>

      {dpError && (
        <div className="rounded-lg border border-danger/40 bg-danger/10 px-4 py-3 text-sm text-red-200">{dpError}</div>
      )}
      ...
      {dpResult && (
        <div className="rounded-lg border border-success/40 bg-success/10 p-4 text-sm">
          <div className="font-semibold text-green-100">
            {dpResult.dryRun
              ? `Preview — ${dpResult.restored} of ${dpResult.traded} traded picks would be restored`
              : <>Wrote <code className="rounded bg-black/30 px-1">{dpResult.output}</code> — {dpResult.restored} picks restored</>}
          </div>
          {dpResult.poolRows === 0 || dpResult.traded === 0 ? (
            <div className="mt-1 text-xs text-amber-200/80">
              No traded future picks in this save ({dpResult.poolRows} pool rows) — nothing to un-trade. This is
              expected unless picks have been traded into the future pool.
            </div>
          ) : (
            <div className="mt-3 max-h-64 overflow-auto rounded-md border border-border bg-surface-1">
              <table className="w-full text-xs">
          ...
          <div className="mt-2 text-xs text-green-200/70">
            {dpResult.dryRun ? 'Looks right? Hit Un-trade to write the save.' : 'Load it in Madden (Franchise → Load).'}
          </div>
        </div>
      )}
