...
import { api, type FranchiseInfo, type FranchisePlayer, type PlayerFieldEdit } from '../api';
...
export function RosterEditor() {
  ...
  useEffect(() => {
    api.franchiseList()
      .then((r) => {
        ...
        const def = r.franchises.find((f) => !/-(CAPRESET|PLAYERS|ROSTER)/i.test(f.name)) || r.franchises[0];
  ...
  async function loadRoster() {
    ...
    try {
      const r = await api.franchisePlayers(selected);
      setTeams(r.teams);
  ...
  const editPlayer = (id: number, patch: PlayerFieldEdit) =>
  ...
  async function apply() {
    if (!selected || editedCount === 0) return;
    setApplying(true); setApplyErr(null); setResult(null);
    try {
      const res = await api.franchiseRosterApply(selected, edits);
      setResult(res);
  ...
  return (
    <div className="mx-auto flex h-full w-full max-w-6xl flex-col gap-3 overflow-hidden px-6 py-5">
      <div className="shrink-0">
        <h1 className="text-xl font-bold tracking-tight">Roster Editor</h1>
        <p className="mt-1 text-xs text-muted">
          Edit any real player in your franchise — ratings, overall, position, age, dev trait. Writes a new{' '}
          <code className="rounded bg-black/30 px-1">CAREER-…-ROSTER</code> file; your original is untouched.
        </p>
      </div>

      <div className="flex shrink-0 flex-wrap items-center gap-2">
        <select value={selected} onChange={(ev) => setSelected(ev.target.value)} className={inputCls}>
          {files.length === 0 && <option value="">No CAREER saves found</option>}
          {files.map((f) => <option key={f.name} value={f.name}>{f.name}</option>)}
        </select>
        <button onClick={loadRoster} disabled={loading || !selected}
      ...
      {players.length > 0 && (
        <div className="grid min-h-0 flex-1 grid-cols-1 gap-3 lg:grid-cols-[minmax(0,1fr)_26rem]">
          ...
          {/* editor */}
          <div className="min-h-0 overflow-auto rounded-lg border border-border bg-surface-1 p-4">
            {!sel ? (
              <div className="flex h-full items-center justify-center text-sm text-neutral-600">Select a player to edit</div>
            ) : (
              <>
                <div className="mb-3">
